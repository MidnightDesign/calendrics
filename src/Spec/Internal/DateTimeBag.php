...
/**
 * Constructing a `PlainDateTime` from a property bag of calendar fields.
 *
 * The bag is already read in TC39 field order by the caller (PrepareCalendarFields via
 * {@see FieldBag}); this class resolves what was read. Resolution order is observable
 * through which error is thrown first: a present monthCode's type and syntax are checked
 * before the year is coerced, era + eraYear override a plain year only on calendars that
 * have eras, and a month given alongside a monthCode must agree with it. Only then does
 * the calendar map the resolved fields to an ISO date, constraining or rejecting
 * per the overflow option.
 *
 * @internal
 */
