...
$options = [(object) ['overflow' => 'constrain'], (object) ['overflow' => 'reject'], (object) [], JsUndefined::singleton()];
foreach ($options as $opt) {
$optionsDesc = $opt && json_encode($opt);
$result = \Calendrics\Spec\PlainMonthDay::from((object) ['year' => 2021, 'month' => 7, 'day' => 3], $opt);
TemporalHelpers::assertPlainMonthDay($result, 'M07', 3, "month 7, day 3, with year, options = {$optionsDesc}");
$result = \Calendrics\Spec\PlainMonthDay::from((object) ['year' => 2021, 'month' => 12, 'day' => 31], $opt);
TemporalHelpers::assertPlainMonthDay($result, 'M12', 31, "month 12, day 31, with year, options = {$optionsDesc}");
$result = \Calendrics\Spec\PlainMonthDay::from((object) ['monthCode' => 'M07', 'day' => 3], $opt);
TemporalHelpers::assertPlainMonthDay($result, 'M07', 3, "monthCode M07, day 3, options = {$optionsDesc}");
$result = \Calendrics\Spec\PlainMonthDay::from((object) ['monthCode' => 'M12', 'day' => 31], $opt);
...
$result = \Calendrics\Spec\PlainMonthDay::from((object) JsUndefined::strip(['month' => $month, 'day' => $daysInMonth]));
TemporalHelpers::assertPlainMonthDay($result, $monthCode, $daysInMonth, "month {$month}, day {$daysInMonth}");
$result = \Calendrics\Spec\PlainMonthDay::from((object) JsUndefined::strip(['monthCode' => $monthCode, 'day' => $daysInMonth]));
