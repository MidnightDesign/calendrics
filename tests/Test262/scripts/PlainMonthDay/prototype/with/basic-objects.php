...
$md = \Calendrics\Spec\PlainMonthDay::from('01-15');
TemporalHelpers::assertPlainMonthDay($md->with((object) ['day' => 22]), 'M01', 22, 'with({day})');
...
Assert::throws(\TypeError::class, function () use (&$md) { return $md->with((object) ['day' => 1, 'timeZone' => 'UTC']); }, 'with({timeZone})');
Assert::throws(\TypeError::class, function () use (&$md) { return $md->with((object) []); }, 'with({})');
Assert::throws(\TypeError::class, function () use (&$md) { return $md->with((object) ['months' => 12]); }, 'with({months})');
TemporalHelpers::assertPlainMonthDay($md->with((object) ['monthCode' => 'M12', 'days' => 1]), 'M12', 15, 'with({monthCode, days})');
