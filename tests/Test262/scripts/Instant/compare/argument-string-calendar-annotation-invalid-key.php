...
Assert::throws(\RangeException::class, function () use (&$arg, &$epoch) { return \Calendrics\Spec\Instant::compare($arg, $epoch); }, "annotation keys must be lowercase: {$arg} - {$descr} (first argument)");
Assert::throws(\RangeException::class, function () use (&$epoch, &$arg) { return \Calendrics\Spec\Instant::compare($epoch, $arg); }, "annotation keys must be lowercase: {$arg} - {$descr} (second argument)");
}
