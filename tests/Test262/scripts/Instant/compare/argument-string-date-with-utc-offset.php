...
// Source: tests/Test262/data/Instant/compare/argument-string-date-with-utc-offset.js
...
$result = \Calendrics\Spec\Instant::compare($arg, $arg);
Assert::sameValue($result, 0, "\"{$arg}\" is a valid UTC offset with time for Instant");
}
...
Assert::throws(\RangeException::class, function () use (&$arg, &$epoch) { return \Calendrics\Spec\Instant::compare($arg, $epoch); }, "\"{$arg}\" UTC offset without time is not valid for Instant (first argument)");
Assert::throws(\RangeException::class, function () use (&$epoch, &$arg) { return \Calendrics\Spec\Instant::compare($epoch, $arg); }, "\"{$arg}\" UTC offset without time is not valid for Instant (second argument)");
}
