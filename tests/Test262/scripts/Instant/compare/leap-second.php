<?php

declare(strict_types=1);

// Source: tests/Test262/data/Instant/compare/leap-second.js
// Generated by tools/transpile-test262.mjs — do not edit manually.
// Re-generate: composer test262:build

use Calendrics\Tests\Test262\Assert;
use Calendrics\Tests\Test262\JsUndefined;
$other = new \Calendrics\Spec\Instant(1_483_228_799_000_000_000);
$arg = '2016-12-31T23:59:60Z';
$result1 = \Calendrics\Spec\Instant::compare($arg, $other);
Assert::sameValue($result1, 0, 'leap second is a valid ISO string for Instant (first argument)');
$result2 = \Calendrics\Spec\Instant::compare($other, $arg);
Assert::sameValue($result2, 0, 'leap second is a valid ISO string for Instant (second argument)');
