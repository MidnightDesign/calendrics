<?php

declare(strict_types=1);

// Source: tests/Test262/data/Instant/prototype/toJSON/negative-epochnanoseconds.js
// Generated by tools/transpile-test262.mjs — do not edit manually.
// Re-generate: composer test262:build

use Calendrics\Tests\Test262\Assert;
use Calendrics\Tests\Test262\JsUndefined;
$datetime = new \Calendrics\Spec\Instant(-13_849_764_999_999_999);
$result = $datetime->toJSON();
Assert::sameValue($result, '1969-07-24T16:50:35.000000001Z', '');
