...
Assert::throws(\RangeException::class, function () use (&$base, &$prop, &$inf, &$overflow) { return \Calendrics\Spec\ZonedDateTime::from(JsUndefined::strip(array_merge($base, [$prop => $inf])), JsUndefined::strip(['overflow' => $overflow])); }, "{$prop} property cannot be {$inf} (overflow {$overflow}");
$calls = new \Calendrics\Tests\Test262\ObserverTrace();
$obj = TemporalHelpers::toPrimitiveObserver($calls, $inf, $prop);
Assert::throws(\RangeException::class, function () use (&$base, &$prop, &$obj, &$overflow) { return \Calendrics\Spec\ZonedDateTime::from(JsUndefined::strip(array_merge($base, [$prop => $obj])), JsUndefined::strip(['overflow' => $overflow])); }, '');
