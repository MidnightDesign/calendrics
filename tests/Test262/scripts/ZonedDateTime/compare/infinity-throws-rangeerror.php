...
Assert::throws(\RangeException::class, function () use (&$base, &$prop, &$inf, &$other) { return \Temporal\Spec\ZonedDateTime::compare(JsUndefined::strip(array_merge($base, [$prop => $inf])), $other); }, "{$prop} property cannot be {$inf}");
Assert::throws(\RangeException::class, function () use (&$other, &$base, &$prop, &$inf) { return \Temporal\Spec\ZonedDateTime::compare($other, JsUndefined::strip(array_merge($base, [$prop => $inf]))); }, "{$prop} property cannot be {$inf}");
$calls1 = new \Temporal\Tests\Test262\ObserverTrace();
$obj1 = TemporalHelpers::toPrimitiveObserver($calls1, $inf, $prop);
Assert::throws(\RangeException::class, function () use (&$base, &$prop, &$obj1, &$other) { return \Temporal\Spec\ZonedDateTime::compare(JsUndefined::strip(array_merge($base, [$prop => $obj1])), $other); }, '');
...
Assert::throws(\RangeException::class, function () use (&$other, &$base, &$prop, &$obj2) { return \Temporal\Spec\ZonedDateTime::compare($other, JsUndefined::strip(array_merge($base, [$prop => $obj2]))); }, '');
