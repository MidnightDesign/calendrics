<?php

declare(strict_types=1);

// Source: tests/Test262/data/ZonedDateTime/prototype/since/zoneddatetime-string-multiple-offsets.js
// Generated by tools/transpile-test262.mjs — do not edit manually.
// Re-generate: composer test262:build

use Calendrics\Tests\Test262\Assert;
use Calendrics\Tests\Test262\JsUndefined;
use Calendrics\Tests\Test262\TemporalHelpers;
$timeZone = '+01:35';
$instance = new \Calendrics\Spec\ZonedDateTime(0, $timeZone);
$str = '1970-01-01T01:35:30+01:35:00.000000000[+01:35]';
$result = $instance->since($str);
TemporalHelpers::assertDuration($result, 0, 0, 0, 0, 0, 0, -30, 0, 0, 0, 'ISO offset, sub-minute offset trailing-zeroes');
$str = '1970-01-01T01:35:30+01:35:00.000000000[+01:35:00.000000000]';
Assert::throws(\RangeException::class, function () use (&$instance, &$str) { return $instance->since($str); }, 'Trailing zeroes not allowed for sub-minute time zone identifiers');
