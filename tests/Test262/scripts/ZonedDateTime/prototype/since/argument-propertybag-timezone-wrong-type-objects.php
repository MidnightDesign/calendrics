...
// Source: tests/Test262/data/ZonedDateTime/prototype/since/argument-propertybag-timezone-wrong-type.js
...
Assert::throws((is_string($timeZone) ? \RangeException::class : \TypeError::class), function () use (&$instance, &$timeZone) { return $instance->since((object) JsUndefined::strip(['year' => 2020, 'month' => 5, 'day' => 2, 'timeZone' => $timeZone])); }, "{$description} does not convert to a valid ISO string");
}
...
[$timeZone, $description] = array_pad($__entry__, 2, null);
Assert::throws(\TypeError::class, function () use (&$instance, &$timeZone) { return $instance->since((object) JsUndefined::strip(['year' => 2020, 'month' => 5, 'day' => 2, 'timeZone' => $timeZone])); }, "{$description} is not a valid object and does not convert to a string");
}
