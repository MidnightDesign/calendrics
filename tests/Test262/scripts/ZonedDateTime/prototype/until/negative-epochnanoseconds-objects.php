<?php

declare(strict_types=1);

// Source: tests/Test262/data/ZonedDateTime/prototype/until/negative-epochnanoseconds.js
// Generated by tools/transpile-test262.mjs — do not edit manually.
// Re-generate: composer test262:build

use Calendrics\Tests\Test262\Assert;
use Calendrics\Tests\Test262\JsUndefined;
use Calendrics\Tests\Test262\TemporalHelpers;
$datetime = new \Calendrics\Spec\ZonedDateTime(-13_849_764_999_999_999, 'UTC');
$result = $datetime->until(new \Calendrics\Spec\ZonedDateTime(0, 'UTC'), (object) ['largestUnit' => 'month']);
TemporalHelpers::assertDuration($result, 0, 5, 0, 7, 7, 9, 24, 999, 999, 999);
