<?php

declare(strict_types=1);

// Source: tests/Test262/data/ZonedDateTime/prototype/yearOfWeek/basic.js
// Generated by tools/transpile-test262.mjs — do not edit manually.
// Re-generate: composer test262:build

use Calendrics\Tests\Test262\Assert;
use Calendrics\Tests\Test262\JsUndefined;
$nsPerDay = 86_400_000_000_000;
$zdt = new \Calendrics\Spec\ZonedDateTime(0, 'UTC');
Assert::sameValue($zdt->yearOfWeek, 1970, '');
$zdt = new \Calendrics\Spec\ZonedDateTime(-3 * $nsPerDay, 'UTC');
Assert::sameValue($zdt->yearOfWeek, 1970, '');
$zdt = new \Calendrics\Spec\ZonedDateTime(-4 * $nsPerDay, 'UTC');
Assert::sameValue($zdt->yearOfWeek, 1969, '');
$zdt = new \Calendrics\Spec\ZonedDateTime(367 * $nsPerDay, 'UTC');
Assert::sameValue($zdt->yearOfWeek, 1970, '');
$zdt = new \Calendrics\Spec\ZonedDateTime(368 * $nsPerDay, 'UTC');
Assert::sameValue($zdt->yearOfWeek, 1971, '');
