...
$dateFromYearMonth = \Calendrics\Spec\PlainDate::from(JsUndefined::strip(['year' => $year, 'month' => $month, 'day' => $day]), $opt);
TemporalHelpers::assertPlainDate($dateFromYearMonth, $year, $month, $monthCode, $day, "{$dateFromYearMonth} - created from year and month");
}
foreach ($options as $opt) {
$dateFromYearMonthCode = \Calendrics\Spec\PlainDate::from(JsUndefined::strip(['year' => $year, 'monthCode' => $monthCode, 'day' => $day]), $opt);
