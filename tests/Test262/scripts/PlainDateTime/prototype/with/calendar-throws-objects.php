<?php

declare(strict_types=1);

// Source: tests/Test262/data/PlainDateTime/prototype/with/calendar-throws.js
// Generated by tools/transpile-test262.mjs — do not edit manually.
// Re-generate: composer test262:build

use Calendrics\Tests\Test262\Assert;
use Calendrics\Tests\Test262\JsUndefined;
$datetime = new \Calendrics\Spec\PlainDateTime(1976, 11, 18, 15, 23, 30, 123, 456, 789);
Assert::throws(\TypeError::class, function () use (&$datetime) { return $datetime->with((object) ['year' => 2021, 'calendar' => 'iso8601']); }, 'throws with calendar property');
