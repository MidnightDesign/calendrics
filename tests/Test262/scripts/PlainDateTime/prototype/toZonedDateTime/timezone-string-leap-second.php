<?php

declare(strict_types=1);

// Source: tests/Test262/data/PlainDateTime/prototype/toZonedDateTime/timezone-string-leap-second.js
// Generated by tools/transpile-test262.mjs — do not edit manually.
// Re-generate: composer test262:build

use Calendrics\Tests\Test262\Assert;
use Calendrics\Tests\Test262\JsUndefined;
$instance = new \Calendrics\Spec\PlainDateTime(2000, 5, 2);
$timeZone = '2016-12-31T23:59:60+00:00[UTC]';
$result = $instance->toZonedDateTime($timeZone);
Assert::sameValue($result->timeZoneId, 'UTC', 'leap second is a valid ISO string for TimeZone');
$timeZone = '2021-08-19T17:30:45.123456789+23:59[+23:59:60]';
Assert::throws(\RangeException::class, function () use (&$instance, &$timeZone) { return $instance->toZonedDateTime($timeZone); }, 'leap second in time zone name not valid');
