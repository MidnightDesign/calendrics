<?php

declare(strict_types=1);

// Source: tests/Test262/data/PlainDateTime/prototype/equals/argument-string-with-utc-designator.js
// Generated by tools/transpile-test262.mjs — do not edit manually.
// Re-generate: composer test262:build

use Calendrics\Tests\Test262\Assert;
use Calendrics\Tests\Test262\JsUndefined;
$invalidStrings = ['2019-10-01T09:00:00Z', '2019-10-01T09:00:00Z[UTC]'];
$instance = new \Calendrics\Spec\PlainDateTime(2000, 5, 2, 12, 34, 56, 987, 654, 321);
foreach ($invalidStrings as $arg) {
Assert::throws(\RangeException::class, function () use (&$instance, &$arg) { return $instance->equals($arg); }, 'String with UTC designator should not be valid as a PlainDateTime');
}
