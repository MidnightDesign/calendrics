<?php

declare(strict_types=1);

// Source: tests/Test262/data/PlainDateTime/compare/leap-second.js
// Generated by tools/transpile-test262.mjs — do not edit manually.
// Re-generate: composer test262:build

use Calendrics\Tests\Test262\Assert;
use Calendrics\Tests\Test262\JsUndefined;
$arg = '2016-12-31T23:59:60';
$result1 = \Calendrics\Spec\PlainDateTime::compare($arg, new \Calendrics\Spec\PlainDateTime(2016, 12, 31, 23, 59, 59));
Assert::sameValue($result1, 0, 'leap second is a valid ISO string for PlainDateTime (first argument)');
$result2 = \Calendrics\Spec\PlainDateTime::compare(new \Calendrics\Spec\PlainDateTime(2016, 12, 31, 23, 59, 59), $arg);
Assert::sameValue($result2, 0, 'leap second is a valid ISO string for PlainDateTime (second argument)');
$arg = (object) ['year' => 2016, 'month' => 12, 'day' => 31, 'hour' => 23, 'minute' => 59, 'second' => 60];
$result3 = \Calendrics\Spec\PlainDateTime::compare($arg, new \Calendrics\Spec\PlainDateTime(2016, 12, 31, 23, 59, 59));
Assert::sameValue($result3, 0, 'second: 60 is constrained in property bag for PlainDateTime (first argument)');
$result4 = \Calendrics\Spec\PlainDateTime::compare(new \Calendrics\Spec\PlainDateTime(2016, 12, 31, 23, 59, 59), $arg);
Assert::sameValue($result4, 0, 'second: 60 is constrained in property bag for PlainDateTime (second argument)');
