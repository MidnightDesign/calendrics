...
Assert::throws(\RangeException::class, function () use (&$base, &$prop, &$inf, &$overflow) { return \Calendrics\Spec\PlainDateTime::from((object) JsUndefined::strip(array_merge((array) $base, [$prop => $inf])), (object) JsUndefined::strip(['overflow' => $overflow])); }, "{$prop} property cannot be {$inf} (overflow {$overflow}");
$calls = new \Calendrics\Tests\Test262\ObserverTrace();
$obj = TemporalHelpers::toPrimitiveObserver($calls, $inf, $prop);
Assert::throws(\RangeException::class, function () use (&$base, &$prop, &$obj, &$overflow) { return \Calendrics\Spec\PlainDateTime::from((object) JsUndefined::strip(array_merge((array) $base, [$prop => $obj])), (object) JsUndefined::strip(['overflow' => $overflow])); }, '');
