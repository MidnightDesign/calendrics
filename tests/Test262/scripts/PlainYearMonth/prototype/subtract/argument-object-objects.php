<?php

declare(strict_types=1);

// Source: tests/Test262/data/PlainYearMonth/prototype/subtract/argument-object.js
// Generated by tools/transpile-test262.mjs — do not edit manually.
// Re-generate: composer test262:build

use Calendrics\Tests\Test262\Assert;
use Calendrics\Tests\Test262\JsUndefined;
use Calendrics\Tests\Test262\TemporalHelpers;
$ym = \Calendrics\Spec\PlainYearMonth::from('2019-11');
$tests = [[(object) ['months' => 2], 2019, 9, 'M09'], [(object) ['years' => 1], 2018, 11, 'M11'], [(object) JsUndefined::strip(['months' => -2]), 2020, 1, 'M01'], [(object) JsUndefined::strip(['years' => -1]), 2020, 11, 'M11']];
foreach ($tests as $__entry_expected__) {
[$argument] = array_pad($__entry_expected__, 1, null);
$expected = array_slice($__entry_expected__, 1);
TemporalHelpers::assertPlainYearMonth($ym->subtract($argument), ...[...$expected, 'no options']);
TemporalHelpers::assertPlainYearMonth($ym->subtract($argument, (object) ['overflow' => 'constrain']), ...[...$expected, 'constrain']);
TemporalHelpers::assertPlainYearMonth($ym->subtract($argument, (object) ['overflow' => 'reject']), ...[...$expected, 'reject']);
}
