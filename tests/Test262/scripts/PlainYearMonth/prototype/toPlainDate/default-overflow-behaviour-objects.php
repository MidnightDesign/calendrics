<?php

declare(strict_types=1);

// Source: tests/Test262/data/PlainYearMonth/prototype/toPlainDate/default-overflow-behaviour.js
// Generated by tools/transpile-test262.mjs — do not edit manually.
// Re-generate: composer test262:build

use Calendrics\Tests\Test262\Assert;
use Calendrics\Tests\Test262\JsUndefined;
use Calendrics\Tests\Test262\TemporalHelpers;
$febCommonYear = new \Calendrics\Spec\PlainYearMonth(2023, 2);
$result = $febCommonYear->toPlainDate((object) ['day' => 29]);
TemporalHelpers::assertPlainDate($result, 2023, 2, 'M02', 28, '2023-02 + 29 = 2023-02-28');
$juneAnyYear = new \Calendrics\Spec\PlainYearMonth(1998, 6);
$result2 = $juneAnyYear->toPlainDate((object) ['day' => 31]);
TemporalHelpers::assertPlainDate($result2, 1998, 6, 'M06', 30, '1998-06 + 31 = 1998-06-31');
