<?php

declare(strict_types=1);

// Source: tests/Test262/data/PlainYearMonth/compare/use-internal-slots.js
// Generated by tools/transpile-test262.mjs — do not edit manually.
// Re-generate: composer test262:build

use Calendrics\Tests\Test262\Assert;
use Calendrics\Tests\Test262\JsUndefined;
$CustomError = function () {
};
// JS-only (getter-only Temporal subclass; getters never fire in PHP, aliased to PlainYearMonth): class AvoidGettersYearMonth extends Temporal.PlainYearMonth { get year() { throw new CustomError(); } get month() { throw new CustomError(); } }
$one = new \Calendrics\Spec\PlainYearMonth(2000, 5);
$two = new \Calendrics\Spec\PlainYearMonth(2006, 3);
Assert::sameValue(\Calendrics\Spec\PlainYearMonth::compare($one, $two), -1, '');
