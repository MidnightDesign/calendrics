<?php

declare(strict_types=1);

// Source: tests/Test262/data/PlainYearMonth/compare/leap-second.js
// Generated by tools/transpile-test262.mjs — do not edit manually.
// Re-generate: composer test262:build

use Calendrics\Tests\Test262\Assert;
use Calendrics\Tests\Test262\JsUndefined;
$arg = '2016-12-31T23:59:60';
$result1 = \Calendrics\Spec\PlainYearMonth::compare($arg, new \Calendrics\Spec\PlainYearMonth(2016, 12));
Assert::sameValue($result1, 0, 'leap second is a valid ISO string for PlainYearMonth (first argument)');
$result2 = \Calendrics\Spec\PlainYearMonth::compare(new \Calendrics\Spec\PlainYearMonth(2016, 12), $arg);
Assert::sameValue($result2, 0, 'leap second is a valid ISO string for PlainYearMonth (second argument)');
$arg = ['year' => 2016, 'month' => 12, 'day' => 31, 'hour' => 23, 'minute' => 59, 'second' => 60];
$result3 = \Calendrics\Spec\PlainYearMonth::compare($arg, new \Calendrics\Spec\PlainYearMonth(2016, 12));
Assert::sameValue($result3, 0, 'second: 60 is ignored in property bag for PlainYearMonth (first argument)');
$result4 = \Calendrics\Spec\PlainYearMonth::compare(new \Calendrics\Spec\PlainYearMonth(2016, 12), $arg);
Assert::sameValue($result4, 0, 'second: 60 is ignored in property bag for PlainYearMonth (second argument)');
