...
Assert::throws(\TypeError::class, function () use (&$year, &$monthCode) { return \Calendrics\Spec\PlainYearMonth::from((object) JsUndefined::strip(['year' => $year, 'monthCode' => $monthCode])); }, ($monthCode instanceof \Calendrics\Tests\Test262\JsSymbol ? 'Symbol should be rejected as month code' : "month code {$monthCode} should be rejected"));
}
