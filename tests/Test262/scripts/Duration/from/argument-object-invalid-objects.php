<?php

declare(strict_types=1);

// Source: tests/Test262/data/Duration/from/argument-object-invalid.js
// Generated by tools/transpile-test262.mjs — do not edit manually.
// Re-generate: composer test262:build

use Calendrics\Tests\Test262\Assert;
use Calendrics\Tests\Test262\JsUndefined;
$tests = [(object) ['years' => 0.5], (object) ['months' => 0.5], (object) ['weeks' => 0.5], (object) ['days' => 0.5], (object) ['hours' => 0.5, 'minutes' => 20], (object) ['hours' => 0.5, 'seconds' => 15], (object) ['minutes' => 10.7, 'nanoseconds' => 400], (object) JsUndefined::strip(['hours' => 1, 'minutes' => -30])];
foreach ($tests as $input) {
Assert::throws(\RangeException::class, function () use (&$input) { return \Calendrics\Spec\Duration::from($input); }, '');
}
Assert::throws(\TypeError::class, fn() => \Calendrics\Spec\Duration::from((object) []), '');
Assert::throws(\TypeError::class, fn() => \Calendrics\Spec\Duration::from((object) ['month' => 12]), '');
