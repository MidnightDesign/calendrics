...
$relativeToYears = JsUndefined::strip(['relativeTo' => $pd, 'largestUnit' => 'years']);
$relativeToMonths = JsUndefined::strip(['relativeTo' => $pd, 'largestUnit' => 'months']);
$relativeToWeeks = JsUndefined::strip(['relativeTo' => $pd, 'largestUnit' => 'weeks']);
$relativeToDays = JsUndefined::strip(['relativeTo' => $pd, 'largestUnit' => 'days']);
Assert::throws(\RangeException::class, function () use (&$duration1, &$pd) { $duration1->round(JsUndefined::strip(['relativeTo' => $pd])); }, 'rounding a year Duration fails without largest/smallest unit');
...
Assert::throws(\RangeException::class, function () use (&$duration2, &$pd) { $duration2->round(JsUndefined::strip(['relativeTo' => $pd])); }, 'rounding a month Duration fails without largest/smallest unit');
...
Assert::throws(\RangeException::class, function () use (&$duration3, &$pd) { $duration3->round(JsUndefined::strip(['relativeTo' => $pd])); }, 'rounding a week Duration fails without largest/smallest unit');
...
Assert::throws(\RangeException::class, function () use (&$duration4, &$pd) { $duration4->round(JsUndefined::strip(['relativeTo' => $pd])); }, 'rounding a day Duration fails without largest/smallest unit');
