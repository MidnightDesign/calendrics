<?php

declare(strict_types=1);

// Source: tests/Test262/data/Duration/prototype/round/smallestunit-undefined.js
// Generated by tools/transpile-test262.mjs — do not edit manually.
// Re-generate: composer test262:build

use Calendrics\Tests\Test262\Assert;
use Calendrics\Tests\Test262\JsUndefined;
use Calendrics\Tests\Test262\TemporalHelpers;
$duration = new \Calendrics\Spec\Duration(0, 0, 0, 0, 1, 2, 3, 123, 456, 789);
$explicit1 = $duration->round((object) ['largestUnit' => 'day']);
TemporalHelpers::assertDuration($explicit1, 0, 0, 0, 0, 1, 2, 3, 123, 456, 789, 'default smallestUnit is nanosecond');
$implicit1 = $duration->round((object) ['largestUnit' => 'day']);
TemporalHelpers::assertDuration($implicit1, 0, 0, 0, 0, 1, 2, 3, 123, 456, 789, 'default smallestUnit is nanosecond');
