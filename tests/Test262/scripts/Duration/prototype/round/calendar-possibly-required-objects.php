...
$duration3 = new \Calendrics\Spec\Duration(0, 0, 5);
$duration4 = new \Calendrics\Spec\Duration(0, 0, 0, 42);
$pd = new \Calendrics\Spec\PlainDate(2021, 12, 15);
$relativeToYears = (object) JsUndefined::strip(['relativeTo' => $pd, 'largestUnit' => 'years']);
$relativeToMonths = (object) JsUndefined::strip(['relativeTo' => $pd, 'largestUnit' => 'months']);
$relativeToWeeks = (object) JsUndefined::strip(['relativeTo' => $pd, 'largestUnit' => 'weeks']);
$relativeToDays = (object) JsUndefined::strip(['relativeTo' => $pd, 'largestUnit' => 'days']);
Assert::throws(\RangeException::class, function () use (&$duration1, &$pd) { $duration1->round((object) JsUndefined::strip(['relativeTo' => $pd])); }, 'rounding a year Duration fails without largest/smallest unit');
...
Assert::throws(\RangeException::class, function () use (&$duration2, &$pd) { $duration2->round((object) JsUndefined::strip(['relativeTo' => $pd])); }, 'rounding a month Duration fails without largest/smallest unit');
...
Assert::throws(\RangeException::class, function () use (&$duration3, &$pd) { $duration3->round((object) JsUndefined::strip(['relativeTo' => $pd])); }, 'rounding a week Duration fails without largest/smallest unit');
...
Assert::throws(\RangeException::class, function () use (&$duration4, &$pd) { $duration4->round((object) JsUndefined::strip(['relativeTo' => $pd])); }, 'rounding a day Duration fails without largest/smallest unit');
