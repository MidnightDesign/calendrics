<?php

declare(strict_types=1);

// Source: tests/Test262/data/Duration/prototype/round/largestunit-undefined.js
// Generated by tools/transpile-test262.mjs — do not edit manually.
// Re-generate: composer test262:build

use Calendrics\Tests\Test262\Assert;
use Calendrics\Tests\Test262\JsUndefined;
use Calendrics\Tests\Test262\TemporalHelpers;
$duration1 = new \Calendrics\Spec\Duration(0, 0, 0, 0, 1, 120, 1, 123, 456, 789);
$explicit1 = $duration1->round((object) ['smallestUnit' => 'nanosecond']);
TemporalHelpers::assertDuration($explicit1, 0, 0, 0, 0, 3, 0, 1, 123, 456, 789, 'default largestUnit is largest in input');
$implicit1 = $duration1->round((object) ['smallestUnit' => 'nanosecond']);
TemporalHelpers::assertDuration($implicit1, 0, 0, 0, 0, 3, 0, 1, 123, 456, 789, 'default largestUnit is largest in input');
$duration2 = new \Calendrics\Spec\Duration(0, 0, 0, 0, 0, 120, 1, 123, 456, 789);
$explicit2 = $duration2->round((object) ['smallestUnit' => 'nanosecond']);
TemporalHelpers::assertDuration($explicit2, 0, 0, 0, 0, 0, 120, 1, 123, 456, 789, 'default largestUnit is largest in input');
$implicit2 = $duration2->round((object) ['smallestUnit' => 'nanosecond']);
TemporalHelpers::assertDuration($implicit2, 0, 0, 0, 0, 0, 120, 1, 123, 456, 789, 'default largestUnit is largest in input');
