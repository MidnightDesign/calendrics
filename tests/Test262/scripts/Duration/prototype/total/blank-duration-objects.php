<?php

declare(strict_types=1);

// Source: tests/Test262/data/Duration/prototype/total/blank-duration.js
// Generated by tools/transpile-test262.mjs — do not edit manually.
// Re-generate: composer test262:build

use Calendrics\Tests\Test262\Assert;
use Calendrics\Tests\Test262\JsUndefined;
$blank = new \Calendrics\Spec\Duration();
$plainRelativeTo = new \Calendrics\Spec\PlainDate(2025, 8, 22);
$zonedRelativeTo = new \Calendrics\Spec\ZonedDateTime(1, 'UTC');
foreach (['days', 'hours', 'minutes', 'seconds', 'milliseconds', 'microseconds', 'nanoseconds'] as $unit) {
$result = $blank->total($unit);
Assert::sameValue($result, 0, "total of {$unit} without relativeTo");
$result = $blank->total((object) JsUndefined::strip(['unit' => $unit, 'relativeTo' => $plainRelativeTo]));
Assert::sameValue($result, 0, "total of {$unit} with PlainDate relativeTo");
$result = $blank->total((object) JsUndefined::strip(['unit' => $unit, 'relativeTo' => $zonedRelativeTo]));
Assert::sameValue($result, 0, "total of {$unit} with ZonedDateTime relativeTo");
}
foreach (['years', 'months', 'weeks'] as $unit) {
$result = $blank->total((object) JsUndefined::strip(['unit' => $unit, 'relativeTo' => $plainRelativeTo]));
Assert::sameValue($result, 0, "total of {$unit} with PlainDate relativeTo");
$result = $blank->total((object) JsUndefined::strip(['unit' => $unit, 'relativeTo' => $zonedRelativeTo]));
Assert::sameValue($result, 0, "total of {$unit} with ZonedDateTime relativeTo");
}
