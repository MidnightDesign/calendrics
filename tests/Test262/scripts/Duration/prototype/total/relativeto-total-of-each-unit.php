...
$totalResults = JsUndefined::strip(['years' => $fullYears + $fractionalYear, 'months' => $fullMonths + $fractionalMonths, 'weeks' => (int) floor($fullDays / 7) + ((2 + $fractionalDay) / 7), 'days' => $fullDays + $fractionalDay, 'hours' => $fullDays * 24 + ($partialDayMilliseconds / 3_600_000), 'minutes' => $fullDays * 24 * 60 + ($partialDayMilliseconds / 60_000), 'seconds' => $fullDays * 24 * 60 * 60 + ($partialDayMilliseconds / 1000), 'milliseconds' => $fullMilliseconds + 0.005_005, 'microseconds' => $fullMilliseconds * 1000 + 5.005, 'nanoseconds' => $fullMilliseconds * 1_000_000 + 5005]);
foreach ($totalResults as $unit => $expected) {
foreach ([$plainRelativeTo, $zonedRelativeTo] as $relativeTo) {
Assert::sameValue($duration->total(JsUndefined::strip(['unit' => $unit, 'relativeTo' => $relativeTo])), $expected, "Duration.total results for {$unit} relative to {$relativeTo}");
}
}
