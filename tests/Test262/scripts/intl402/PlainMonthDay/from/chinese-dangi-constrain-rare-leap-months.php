...
$nonLeapMonthCode = \Calendrics\Tests\Test262\Js::slice($monthCode, 0, -1);
$pmd = \Calendrics\Spec\PlainMonthDay::from(JsUndefined::strip(['calendar' => $calendar, 'monthCode' => $monthCode, 'day' => $day]));
$constrain = \Calendrics\Spec\PlainMonthDay::from(JsUndefined::strip(['calendar' => $calendar, 'monthCode' => $nonLeapMonthCode, 'day' => 30]));
Assert::sameValue($constrain->equals($pmd), true, '');
}
}
}
$nonexistentMonthCodesWithYears = [['monthCode' => 'M01L', 'referenceYear' => 1972], ['monthCode' => 'M12L', 'referenceYear' => 1972]];
foreach ($calendars as $calendar) {
foreach ($nonexistentMonthCodesWithYears as $__obj__) {
$monthCode = $__obj__['monthCode'] ?? null;
$referenceYear = $__obj__['referenceYear'] ?? null;
$nonLeapMonthCode = \Calendrics\Tests\Test262\Js::slice($monthCode, 0, -1);
$pmd = \Calendrics\Spec\PlainMonthDay::from(JsUndefined::strip(['calendar' => $calendar, 'monthCode' => $monthCode, 'day' => 29]));
$constrain = \Calendrics\Spec\PlainMonthDay::from(JsUndefined::strip(['calendar' => $calendar, 'monthCode' => $nonLeapMonthCode, 'day' => 29]));
