<?php

declare(strict_types=1);

// Source: tests/Test262/data/intl402/PlainMonthDay/prototype/toString/calendarname-never.js
// Generated by tools/transpile-test262.mjs — do not edit manually.
// Re-generate: composer test262:build

use Calendrics\Tests\Test262\Assert;
use Calendrics\Tests\Test262\JsUndefined;
$tests = [[[], '05-02', 'built-in ISO'], [['gregory'], '1972-05-02', 'built-in Gregorian']];
foreach ($tests as $__entry__) {
[$args, $expected, $description] = array_pad($__entry__, 3, null);
$monthday = new \Calendrics\Spec\PlainMonthDay(5, 2, ...$args);
$result = $monthday->toString((object) ['calendarName' => 'never']);
Assert::sameValue($result, $expected, "{$description} calendar for calendarName = never");
}
