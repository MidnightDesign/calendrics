...
$result = \Calendrics\Spec\PlainYearMonth::from($fields, (object) ['overflow' => 'constrain']);
...
Assert::throws(\RangeException::class, function () use (&$fields) { return \Calendrics\Spec\PlainYearMonth::from($fields, (object) ['overflow' => 'reject']); }, "Chinese intercalary month {$leapMonthCode} does not exist in year 2022 (overflow reject)");
}
...
$result = \Calendrics\Spec\PlainYearMonth::from((object) JsUndefined::strip(['year' => $year, 'monthCode' => $monthCode, 'calendar' => 'chinese']));
...
$isoYearMonth = \Calendrics\Tests\Test262\Js::slice((string) ($result), 0, 7);
Assert::sameValue($isoYearMonth, "{$isoYear}-" . (str_pad(\Calendrics\Tests\Test262\Js::toString($isoMonth), 2, '0', STR_PAD_LEFT)) . "", "{$year}-{$monthCode} starts in ISO month {$isoYear}-{$isoMonth}");
}
