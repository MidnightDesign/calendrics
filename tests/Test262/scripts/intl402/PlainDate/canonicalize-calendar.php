<?php

declare(strict_types=1);

// Source: tests/Test262/data/intl402/PlainDate/canonicalize-calendar.js
// Generated by tools/transpile-test262.mjs — do not edit manually.
// Re-generate: composer test262:build

use Calendrics\Tests\Test262\Assert;
use Calendrics\Tests\Test262\JsUndefined;
$result = new \Calendrics\Spec\PlainDate(2024, 7, 2, 'islamicc');
Assert::sameValue($result->calendarId, 'islamic-civil', 'calendar ID is canonicalized');
$result = new \Calendrics\Spec\PlainDate(2024, 7, 2, 'ethiopic-amete-alem');
Assert::sameValue($result->calendarId, 'ethioaa', 'calendar ID is canonicalized');
