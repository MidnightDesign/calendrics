<?php

declare(strict_types=1);

// Source: tests/Test262/data/intl402/PlainDate/prototype/toString/calendarname-never.js
// Generated by tools/transpile-test262.mjs — do not edit manually.
// Re-generate: composer test262:build

use Calendrics\Tests\Test262\Assert;
use Calendrics\Tests\Test262\JsUndefined;
$tests = [[[], '2000-05-02', 'built-in ISO'], [['gregory'], '2000-05-02', 'built-in Gregorian']];
foreach ($tests as $__entry__) {
[$args, $expected, $description] = array_pad($__entry__, 3, null);
$date = new \Calendrics\Spec\PlainDate(2000, 5, 2, ...$args);
$result = $date->toString((object) ['calendarName' => 'never']);
Assert::sameValue($result, $expected, "{$description} calendar for calendarName = never");
}
