<?php

declare(strict_types=1);

// Source: tests/Test262/data/intl402/PlainDateTime/canonicalize-calendar.js
// Generated by tools/transpile-test262.mjs — do not edit manually.
// Re-generate: composer test262:build

use Calendrics\Tests\Test262\Assert;
use Calendrics\Tests\Test262\JsUndefined;
$result = new \Calendrics\Spec\PlainDateTime(2024, 7, 2, 12, 34, 56, 987, 654, 321, 'islamicc');
Assert::sameValue($result->calendarId, 'islamic-civil', 'calendar ID is canonicalized');
$result = new \Calendrics\Spec\PlainDateTime(2024, 7, 2, 12, 34, 56, 987, 654, 321, 'ethiopic-amete-alem');
Assert::sameValue($result->calendarId, 'ethioaa', 'calendar ID is canonicalized');
