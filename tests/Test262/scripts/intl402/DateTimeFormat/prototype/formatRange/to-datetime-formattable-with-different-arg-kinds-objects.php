...
// Source: tests/Test262/data/intl402/DateTimeFormat/prototype/formatRange/to-datetime-formattable-with-different-arg-kinds.js
...
// JS-only (references JS-only ToPrimitive tracker variable): assert.throws(TypeError, function() { dtf.formatRange(invalidDateValue, object); });
// JS-only (references JS-only ToPrimitive tracker variable): assert.sameValue(callCount, i * 2 + 1);
// JS-only (references JS-only ToPrimitive tracker variable): assert.throws(TypeError, function() { dtf.formatRange(object, invalidDateValue); });
