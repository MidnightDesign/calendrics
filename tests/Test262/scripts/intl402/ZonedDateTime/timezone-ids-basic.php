<?php

declare(strict_types=1);

// Source: tests/Test262/data/intl402/ZonedDateTime/timezone-ids-basic.js
// Generated by tools/transpile-test262.mjs — do not edit manually.
// Re-generate: composer test262:build

use Calendrics\Tests\Test262\Assert;
use Calendrics\Tests\Test262\JsUndefined;
$valid = [['Europe/Vienna'], ['America/New_York'], ['Africa/CAIRO', 'Africa/Cairo'], ['africa/cairo', 'Africa/Cairo'], ['Asia/Ulaanbaatar'], ['Asia/Ulan_Bator'], ['UTC'], ['GMT']];
foreach ($valid as $__entry__) {
[$zone, $id] = array_pad($__entry__, 2, null);
$id = $id ?? $zone;
$result = new \Calendrics\Spec\ZonedDateTime(0, $zone);
Assert::sameValue((is_object($result) && !($result instanceof \Calendrics\Tests\Test262\JsSymbol)), true, "object should be created for {$zone}");
Assert::sameValue($result->timeZoneId, $id, "id for {$zone} should be {$id}");
}
$invalid = ['+00:01.1', '-01.1'];
foreach ($invalid as $zone) {
Assert::throws(\RangeException::class, function () use (&$zone) { return new \Calendrics\Spec\ZonedDateTime(0, $zone); }, "should throw for {$zone}");
}
