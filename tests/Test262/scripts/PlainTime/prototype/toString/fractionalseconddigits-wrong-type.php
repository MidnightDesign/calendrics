...
$time = new \Calendrics\Spec\PlainTime(12, 34, 56, 987, 650, 0);
Assert::throws(\RangeException::class, function () use (&$time) { return $time->toString(['fractionalSecondDigits' => null]); }, 'null is not a number and converts to the string \'null\' which is not valid for fractionalSecondDigits');
Assert::throws(\RangeException::class, function () use (&$time) { return $time->toString(['fractionalSecondDigits' => true]); }, 'true is not a number and converts to the string \'true\' which is not valid for fractionalSecondDigits');
Assert::throws(\RangeException::class, function () use (&$time) { return $time->toString(['fractionalSecondDigits' => false]); }, 'false is not a number and converts to the string \'false\' which is not valid for fractionalSecondDigits');
Assert::throws(\TypeError::class, function () use (&$time) { return $time->toString(JsUndefined::strip(['fractionalSecondDigits' => \Calendrics\Tests\Test262\JsSymbol::singleton()])); }, 'symbols are not numbers and cannot convert to strings');
