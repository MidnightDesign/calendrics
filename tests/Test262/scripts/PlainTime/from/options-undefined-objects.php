<?php

declare(strict_types=1);

// Source: tests/Test262/data/PlainTime/from/options-undefined.js
// Generated by tools/transpile-test262.mjs — do not edit manually.
// Re-generate: composer test262:build

use Calendrics\Tests\Test262\Assert;
use Calendrics\Tests\Test262\JsUndefined;
$fields = (object) ['hour' => 12, 'minute' => 60];
$explicit = \Calendrics\Spec\PlainTime::from($fields);
Assert::sameValue($explicit->minute, 59, 'default overflow is constrain');
$implicit = \Calendrics\Spec\PlainTime::from($fields);
Assert::sameValue($implicit->minute, 59, 'default overflow is constrain');
$lambda = \Calendrics\Spec\PlainTime::from($fields, function () {  });
Assert::sameValue($lambda->minute, 59, 'default overflow is constrain');
