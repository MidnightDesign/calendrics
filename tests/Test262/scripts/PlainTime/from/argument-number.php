<?php

declare(strict_types=1);

// Source: tests/Test262/data/PlainTime/from/argument-number.js
// Generated by tools/transpile-test262.mjs — do not edit manually.
// Re-generate: composer test262:build

use Calendrics\Tests\Test262\Assert;
use Calendrics\Tests\Test262\JsUndefined;
$numbers = [1, -123_456.987_654_321, 1_234_567, 123_456.9_876_543_219];
foreach ($numbers as $arg) {
Assert::throws(\TypeError::class, function () use (&$arg) { return \Calendrics\Spec\PlainTime::from($arg); }, "A number ({$arg}) is not a valid ISO string for PlainTime");
}
