...
Assert::throws(\TypeError::class, fn() => \Calendrics\Spec\PlainTime::from(), 'no argument');
...
[$arg, $description] = array_pad($__entry__, 2, null);
Assert::throws(\TypeError::class, function () use (&$arg) { return \Calendrics\Spec\PlainTime::from($arg); }, "{$description} is not a valid property bag and does not convert to a string");
foreach ([JsUndefined::singleton(), (object) ['overflow' => 'constrain'], (object) ['overflow' => 'reject']] as $options) {
Assert::throws(\TypeError::class, function () use (&$arg, &$options) { return \Calendrics\Spec\PlainTime::from($arg, $options); }, "{$description} is not a valid property bag and does not convert to a string with options " . json_encode($options) . "");
}
}
