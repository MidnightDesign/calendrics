<?php

declare(strict_types=1);

// Source: tests/Test262/data/PlainTime/compare/use-internal-slots.js
// Generated by tools/transpile-test262.mjs — do not edit manually.
// Re-generate: composer test262:build

use Calendrics\Tests\Test262\Assert;
use Calendrics\Tests\Test262\JsUndefined;
$CustomError = function () {
};
// JS-only (getter-only Temporal subclass; getters never fire in PHP, aliased to PlainTime): class AvoidGettersTime extends Temporal.PlainTime { get hour() { throw new CustomError(); } get minute() { throw new CustomError(); } get second() { throw new …
$one = new \Calendrics\Spec\PlainTime(12, 34, 56, 987, 654, 321);
$two = new \Calendrics\Spec\PlainTime(6, 54, 32, 123, 456, 789);
Assert::sameValue(\Calendrics\Spec\PlainTime::compare($one, $two), 1, '');
