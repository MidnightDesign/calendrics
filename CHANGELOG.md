...
- **`ZonedDateTime::getTimeZoneTransition('next')` no longer reports a historical transition on newer tzdata.** With recent tzdata/PHP builds, `DateTimeZone::getTransitions($begin, $end)` can return a zone's full history when its only offset change predates the requested range (e.g. Asia/Riyadh's 1947 LMT switch); the "next" scan then surfaced that 1947 transition as if it were upcoming. The scan now requires a candidate to lie strictly after the anchor instant — a no-op on runtimes where `getTransitions()` honors its range. This was breaking CI on `master` (test262 `getTimeZoneTransition/no-transitions-named-timezone`).
