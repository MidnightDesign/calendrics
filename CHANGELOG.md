...
- **`Temporal\DateTimeFormat` — locale-aware formatting for the porcelain layer.** Until now the only way to render a porcelain value in a locale was to drop to the spec layer and hand it an ECMA-402 option bag of magic strings (`$date->toSpec()->toLocaleString('de-AT', ['dateStyle' => 'full'])`) — precisely the stringly-typed shape the porcelain layer exists to remove. `DateTimeFormat` is an immutable, reusable formatter that takes backed enums instead: `DateTimeFormat::styled('de-AT', date: DateStyle::Full, time: TimeStyle::Short)->format($date)`. It formats `PlainDate`, `PlainTime`, `PlainDateTime`, `PlainYearMonth`, `PlainMonthDay`, `Instant`, and `ZonedDateTime`, and delegates to the spec layer, so the ECMA-402 semantics — calendar compatibility, style/component conflicts, the per-type option rules — are the ones test262 already covers. ECMA-402 forbids combining the `dateStyle`/`timeStyle` presets with individual component options; the two live on separate constructors (`styled()` and `components()`) so that combination is unrepresentable rather than merely rejected at runtime. New enums: `DateStyle`, `TimeStyle`, `TextWidth`, `NumericWidth`, `MonthWidth`, `TimeZoneNameStyle`, `HourCycle`.
